import { resolveGraveFortunesRound, simulate, mulberry32, simulateAttemptsWithBullets, attemptModel } from './game';
...
if (opts.simulate) {
  ...
  const result = simulate(rounds, stake, rng);
...
if (opts.attemptSim) {
  ...
  const seed = opts.seed ? parseInt(opts.seed, 10) : Date.now();
  const rng = mulberry32(seed);
  const sim = simulateAttemptsWithBullets(rounds, stake, goldProb, rng, attemptModel);
...
const result = resolveGraveFortunesRound(stake);
